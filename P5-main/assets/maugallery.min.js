...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){let activeImage=null;let imagesCollection=[];let activeTag=$(".tags-bar span.active-tag").data("images-toggle");if(activeTag==="all"){imagesCollection=$(".gallery-item").toArray()}else{imagesCollection=$(".gallery-item[data-gallery-tag='"+activeTag+"']").toArray()}
$(imagesCollection).each(function(i){if($(this).find("img").attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this).find("img")}});let index=imagesCollection.indexOf(activeImage.closest(".gallery-item")[0]);let prevIndex=(index===0)?imagesCollection.length-1:index-1;$(".lightboxImage").attr("src",$(imagesCollection[prevIndex]).find("img").attr("src"))},nextImage(lightboxId){let activeImage=null;let imagesCollection=[];let activeTag=$(".tags-bar span.active-tag").data("images-toggle");if(activeTag==="all"){imagesCollection=$(".gallery-item").toArray()}else{imagesCollection=$(".gallery-item[data-gallery-tag='"+activeTag+"']").toArray()}
$(imagesCollection).each(function(i){if($(this).find("img").attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this).find("img")}});let index=imagesCollection.indexOf(activeImage.closest(".gallery-item")[0]);let nextIndex=(index===imagesCollection.length-1)?0:index+1;$(".lightboxImage").attr("src",$(imagesCollection[nextIndex]).find("img").attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${lightboxId || 'galleryLightbox'}" tabindex="-1" role="dialog" aria-hidden="true">
...
$(".active-tag").removeClass("active active-tag").css("background-color","");$(this).addClass("active-tag").css("background-color","BEB45A");var tag=$(this).data("images-toggle");$(".gallery-item").each(function(){$(this).parents(".item-column").hide();if(tag==="all"){$(this).parents(".item-column").show(300)}else if($(this).data("gallery-tag")===tag){$(this).parents(".item-column").show(300)}})}}})(jQuery)